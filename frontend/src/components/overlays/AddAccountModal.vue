    ...
    <section class="account-modal">
      ...
      <form class="setup-form" @submit.prevent="submit">
        ...
        <template v-if="s.setup.value.method === 'imap'">
          <label><span>Password</span><input v-model="s.setup.value.appPassword" type="password" autocomplete="current-password" placeholder="mailbox password" required /></label>
